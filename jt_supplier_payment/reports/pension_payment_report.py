...
class SalariesAlimonyAndAdditionalBenefit(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        domain = [('invoice_date','>=',start),('invoice_date','<=',end),('is_pension_payment_request','=',True),('payment_state','=','approved_payment')]
...
#             domain += [('journal_id','in',journal_list)]
        ...
        journal_ids = records.mapped('payment_bank_account_id')
        total_amount = 0
        
        for journal in journal_ids:
            payment_method_ids = records.filtered(lambda a:a.payment_bank_account_id.id==journal.id and a.l10n_mx_edi_payment_method_id).mapped('l10n_mx_edi_payment_method_id')
            total_bank = 0
            if payment_method_ids:
                lines.append({
                    'id': 'hierarchy1_' + str(journal.id),
                    'name': journal.bank_id and journal.bank_id.name or '',
                    'columns': [
                                {'name':journal.acc_number or ''},
                                {'name':''},
                                ],
                    'level': 1,
                    'unfoldable': False,
            ...
            for method in payment_method_ids:
                ...
                amount = sum(x.amount_total for x in records.filtered(lambda a:a.payment_bank_account_id.id==journal.id and a.l10n_mx_edi_payment_method_id.id==method.id))
