...
class AccountMoveLine(models.Model):
    ...
    payment_bank_reference = fields.Char(related='payment_id.bank_reference')
...
class AccountReconciliation(models.AbstractModel):
    ...
    def _get_statement_line(self, st_line):
        data = super(AccountReconciliation,self)._get_statement_line(st_line)
        data.update({'bank_reference':st_line.bank_reference and st_line.bank_reference or ''})
        return data

    @api.model
    def _prepare_move_lines(self, move_lines, target_currency=False, target_date=False, recs_count=0):
        datas = super(AccountReconciliation,self)._prepare_move_lines(move_lines, target_currency=target_currency, target_date=target_date, recs_count=recs_count)
        
        for data in datas:
            if data.get('id',0):
                aml_rec = self.env['account.move.line'].browse(data.get('id',0))
                if aml_rec.debit > 0:
                    data.update({'payment_bank_reference':aml_rec and aml_rec.payment_bank_reference and aml_rec.payment_bank_reference or ''})
                    ...
                    for c in available_candidates:
                        ...
                        if float_is_zero(residual_amount - line_residual, precision_rounding=line_currency.rounding):
                            if line.bank_reference:
                                custom_move_line = self.env['account.move.line'].browse(c.get('aml_id'))
                                if custom_move_line and custom_move_line.payment_bank_reference and custom_move_line.payment_bank_reference==line.bank_reference:
