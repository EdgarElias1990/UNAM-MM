...
class TotalPaymentAmounts(models.AbstractModel):
    ...
    def _get_filter_dependancy(self):
        return self.env['dependency'].search([],order='dependency')
    ...
    def _get_lines(self, options, line_id=None):
        ...
        domain = [('invoice_date','>=',start),('invoice_date','<=',end),('payment_place_id','!=',False),('is_payroll_payment_request','=',True),('payment_state','=','approved_payment')]
...
#             dependency_list = [0]
        ...
        for dep in records.mapped('dependancy_id').sorted(key='dependency'):

            lines.append({
                'id': 'hierarchy1_dep' + str(dep.id),
            ...
            lines.append({
                'id': 'hierarchy1_dep_blank' + str(dep.id),
                'name': '',
                'level': 1,
                'unfoldable': False,
                'unfolded': True,
            ...
            current_dep_records = records.filtered(lambda a:a.dependancy_id.id==dep.id)
            for sub_dep in current_dep_records.mapped('sub_dependancy_id').sorted(key='sub_dependency'):

                lines.append({
                    'id': 'hierarchy1_dep' + str(sub_dep.id),
                    'name': sub_dep.description,
                    'level': 3,
                    'unfoldable': False,
                    'unfolded': True,
                    'colspan' : 3,
                ...
                lines.append({
                    'id': 'hierarchy1_dep_blank' + str(sub_dep.id),
                    'name': '',
                    'level': 1,
                    'unfoldable': False,
                    'unfolded': True,
                ...
                lines.append({
                    'id': 'hierarchy1_dep_col' + str(sub_dep.id),
                    'name': 'Lugar de pago',
                    'level': 1,
                    'columns': [
                                    {'name':'Descripción del lugar de pago'},
                ...
                current_subdep_records = current_dep_records.filtered(lambda a:a.sub_dependancy_id.id==sub_dep.id)
                payment_place_ids = current_subdep_records.mapped('payment_place_id')
                total_amount = 0
                
                for place_id in payment_place_ids:
                    amount = sum(x.amount_total for x in current_subdep_records.filtered(lambda a:a.payment_place_id.id==place_id.id))
                ...
                lines.append({
                    'id': 'hierarchy1_total_blank' + str(sub_dep.id),
                    'name': '',
                    'level': 5,
                    'unfoldable': False,
                    'unfolded': True,
        ...
        if lines:
            lines.append({
                'id': 'hierarchy1_grand_total',
                'name': '',
                'columns': [
                            {'name':'TOTAL'},
                            self._format({'name': grand_total_amount},figure_type='float'),
                            ],
                'level': 1,
                'unfoldable': False,
                'unfolded': True,
